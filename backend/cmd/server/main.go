...
func main() {
	...
	mux.HandleFunc("GET /api/v1/tasks/mine", auth(handler.HandleListMyTasks(db)))
	mux.HandleFunc("GET /api/v1/tasks/by-ref/{ref}", auth(handler.HandleGetTaskByRef(db)))
	...
	mux.HandleFunc("POST /api/v1/projects/{projectId}/tasks/{taskId}/handoff", auth(handler.HandleHandoffTask(db)))
